...
#         self.local_conn = None  # Initialize as None
...
#         print("Step 5: Setting up database in background...")
#         # Setup database in background thread to avoid blocking UI
#         self.root.after(100, self.setup_database_pool)

#         print("Step 6: App ready!")
...
#             if hasattr(self, "sync_status_label"):
#                 self.sync_status_label.config(text=f"DB Error {e}", foreground="red")
...
#         """Browse for CSV file with UI update"""
...
#         # Show loading status
#         if hasattr(self, "sync_status_label"):
#             self.sync_status_label.config(text="Uploading CSV...", foreground="blue")

#         # Run CSV import in background thread
#         import_thread = threading.Thread(
#             target=self._import_csv_worker, args=(file_path,), daemon=True
#         )
#         import_thread.start()

#     def _import_csv_worker(self, file_path):
...
#                     self.root.after(0, lambda: self.display_import_results(result))
#                 else:
#                     self.root.after(
#                         0,
#                         lambda: messagebox.showerror(
#                             "Error", f"Import failed: {response.text}"
#                         ),
#                     )

#         except Exception as e:
#             self.root.after(
#                 0, lambda: messagebox.showerror("Error", f"Import error: {str(e)}")
#             )
#         finally:
...
#         # Add a commit button instead of blocking messagebox
#         self.results_text.insert(tk.END, "\n" + "=" * 50 + "\n")
#         self.results_text.insert(
#             tk.END,
#             "Click 'Commit Transactions' button below to save these transactions.\n",
#         )
...
#                 command=self.commit_pending_transactions,
#             )
...
#             self.commit_transactions(self.pending_transactions)
#             # Hide the commit button after successful commit
