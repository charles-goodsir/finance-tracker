...
def lambda_handler(event, context):
    ...
    try:
        ...
        if processed_count > 0:
            send_telegram_notification(f"🤖 *Recurring Transactions Processed*\n\nProcessed {processed_count} recurring transactions today!")
        ...
        send_telegram_notification(f"❌ *Error Processing Recurring Transactions*\n\n{str(e)}")
        return {
            'statusCode': 500,
            'body': json.dumps({
                'error': str(e)
    ...
    if recurring.get('end_date'):
        ...
        if next_due > end_date:
            ...
            recurring_table.update_item(
                Key={'user_id': recurring['user_id'], 'recurring_id': recurring['recurring_id']},
                UpdateExpression='SET is_active = :inactive',
                ExpressionAttributeValues={':inactive': False}
            )
            send_telegram_notification(f"⏹️ *Recurring Transaction Ended*\n\n{recurring['description']} has reached its end date and has been deactivated.")
